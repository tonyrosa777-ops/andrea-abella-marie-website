...
export async function POST(req: NextRequest) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    // Build Printful order items — map cart items to sync variant IDs
    ...
    if (printfulItems.length === 0) {
      console.warn("No Printful variant IDs found in cart for session:", session.id);
      return NextResponse.json({ received: true });
    }

    try {
      const order = await createOrder(seededProducts.storeId, {
        recipient: {
